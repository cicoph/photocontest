			...
			if (Yii::app()->user->isGuest) {
				...
				if (isset($_POST['User'])) {
					...
					if ($user->validate() && $user->save()) {
						if ($this->module->withYiiUser == true) {
							$profile = new Profile();
							$profile->first_name=$identity->loginProviderFirstName;
							$profile->last_name=$identity->loginProviderLastName;
							$profile->user_id=$user->id;
							$profile->save();
						}						
						$identity->id = $user->id;
						$identity->username = $user->username;
						$permission = Rights::module()->authenticatedName;
						Rights::assign($permission[1], $identity->id);
				...
				$this->render('createUser', array(
					'user' => $user,
					'username'=>$identity->loginProviderUserName,
					'email' => $identity->loginProviderEmail,
				));
			} else {
